...
export class TaskStatusService {
  private taskStatusSubject = new BehaviorSubject<{
    status: string;
    points: number;
    otherTasks?: string;
  ...
  setTaskStatus(
    ...
    if (confirmed) {
      ...
      console.log(
        `Total points updated. User ID: ${userId}, Total Points: ${points}`
      );
    }
      ...
      await this.firestore.firestore.runTransaction(async (transaction) => {
        ...
        if (userDoc.exists) {
          const currentTotalPoints =
            (userDoc.data() as { totalPoints?: number })?.totalPoints || 0;
          ...
          console.log(
            `Total points updated in Firestore. User ID: ${userId}, Total Points: ${newTotalPoints}`
          );
        } else {
